...
<template>
  <div>
    <zy-filter-bar ref="filter" @exec="exec" :result-count="count" size="default">
      <template #tl>
      ...
      <template #tr="condition">
        ...
        <zy-filter-input v-model="quick" placeholder="Please input" @change="change"/>
      </template>
      <template #default>
        <zy-filter-select size="default" label="事件类型" field="type" :options="options" multiple clearable/>
        <zy-filter-date size="default" label="操作日期" field="date"/>
        <zy-filter-date-time size="default" label="操作时间" field="datetime"/>
      </template>
      <template #buttons>
        <zy-table-column-select type="button" :columns="columns" size="default"/>
      </template>
    </zy-filter-bar>
  </div>
