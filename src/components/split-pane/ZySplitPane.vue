<template>
  <div :style="{ cursor, userSelect }" class="zy-split-pane" ref="outerWrapper">
    <div :class="[`is-${direction}`, 'zy-split-pane__left']"
    ...
    <div :class="[`is-${direction}`, 'zy-split-pane__right']"
...
defineOptions({ name: "ZySplitPane" });
...
const saveKey = computed(() => {
  return "Zy-SP-" + props.localKey;
})
const resizerClasses = computed(() => {
  const classes = [
    `zy-split-pane__${props.resizerType}`,
    `is-${props.direction}`,
