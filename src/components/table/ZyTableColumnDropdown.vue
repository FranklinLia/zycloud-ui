<template>
  <el-table-column class-name="zy-table-column-dropdown" :width="width" :resizable="false" align="center"
    v-bind="$attrs">
    <template #default="scope">
      <el-dropdown @command="handleCommand" :trigger="trigger" :class="showType === 'hover' ? 'zy-show-icon' : ''"
        v-if="isShow(scope.row)" placement="bottom" :ref="`dropdown${scope.$index}`">
        <span class="zy-dropdown-link">
          <slot name="icon">
            <el-icon class="zy-icon-more">
              <MoreFilled />
            </el-icon>
          </slot>
        </span>
        <template #dropdown>
          <el-dropdown-menu>
            <slot name="title">
              <div class="zy-table-column-dropdown__title" v-if="title">
                {{ title }}
              </div>
            </slot>
            <slot v-bind="scope">
...
defineOptions({ name: "ZyTableColumnDropdown" });
