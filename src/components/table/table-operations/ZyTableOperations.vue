<template>
  <el-table-column class-name="zy-table-operations" :align="align" :width="computeWidth" v-bind="$attrs">
    <template #header>
      {{ $attrs.label }}
    </template>
    <template #default="{ row }">
      <zy-table-button :icon="type === 'icon'" v-for="(btn, i) in defaultButtons(row)" :key="i"
      ...
      <zy-table-more-button :type="type" :buttons="moreButtons(row)" :row="row" v-if="moreButtons(row).length > 0" />
    </template>
  </el-table-column>
</template>
...
import ZyTableButton from "./ZyTableButton.vue";
import ZyTableMoreButton from "./ZyTableMoreButton.vue";

defineOptions({ name: "ZyTableOperations" });
...
const computeWidth = computed(() => {
  let length = hasShowZync.value ? props.ellipsis : defaultButtons.value.length
